...
fn build_matrix_proof(matrix: &[Vec<i8>], job_key: &[u8; 32], row_indices: &[usize], num_cols: usize) -> MatrixMerkleProof {
    ...
    let leaf_indices =
        pearl_blake3::MerkleTree::compute_leaf_indices_from_rows(row_indices, (matrix.len(), num_cols), BLAKE3_CHUNK_LEN)
            .expect("1024 is in ALLOWED_CHUNK_LENS");
...
fn build_routing_proof(
    ...
    let leaf_indices = pearl_blake3::MerkleTree::compute_leaf_indices_from_rows(
        &row_indices,
        (total_entries, std::mem::size_of::<u32>()),
        BLAKE3_CHUNK_LEN,
    )
    .expect("1024 is in ALLOWED_CHUNK_LENS");
    let proof = tree.get_multileaf_proof(&leaf_indices);
...
mod tests {
    ...
    use crate::v2::api::{proof::MMAType, verify::verify_plain_proof};
