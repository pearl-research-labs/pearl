...
in `Oyster` to help improve the overall readability.

## Spacing

Blocks of code within `Oyster` should be segmented into logical stanzas of
...
When making use of structured logging calls (any `Logger` methods ending in
`S`), a few different rules and exceptions apply.
