...
// pendingTxDetails loads txHash and rejects anything that is not a pending transaction of this wallet.
func (w *Wallet) pendingTxDetails(ns walletdb.ReadBucket, txHash chainhash.Hash) (*wtxmgr.TxDetails, error) {
...
// RemoveTransaction forgets the pending transaction txHash together with every pending transaction that spends from it,
// so the inputs they consumed become spendable again. It returns the removed hashes, txHash first.
//
// The network is not consulted: a peer that already holds the transaction may still mine it, in which case spending the
// freed inputs again is a double-spend attempt. Callers must have warned the user before calling.
func (w *Wallet) RemoveTransaction(txHash chainhash.Hash) ([]chainhash.Hash, error) {
...
// RebroadcastTransaction announces the pending transaction txHash to the network again, preceded by any of its
// ancestors that are still pending, in dependency order. Nothing re-announces a parent on its own, so a child whose
// parent no peer holds would otherwise stay an orphan.
//
...
// unminedAncestry returns the transaction txHash preceded by every transaction in unmined that it spends from, directly
// or through other unmined transactions. unmined must be in dependency order, as TxStore.UnminedTxs guarantees: walked
// backwards, every child comes before its parents.
func unminedAncestry(txHash chainhash.Hash, unmined []*wire.MsgTx) []*wire.MsgTx {
