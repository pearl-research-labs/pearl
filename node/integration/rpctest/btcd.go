...
func pearldExecutablePath() (string, error) {
	...
	// Concurrent `go test` processes share testDir. Building straight into
	// the shared path lets two `go build` invocations interleave and leave a
	// truncated binary, so build privately and publish with an atomic rename.
	buildPath := fmt.Sprintf("%s.%d.tmp", outputPath, rand.Uint32())
	cmd := exec.Command(
		"go", "build", "-o", buildPath, "github.com/pearl-research-labs/pearl/node",
	)
	err = cmd.Run()
	if err != nil {
