...
const (
	...
	// Every fake peer originates from 127.0.0.1, so one wave cannot exceed
	// the node's per-source pending-handshake budget, and the per-source v2
	// handshake rate (2/s, burst 4) bounds how fast a wave can be admitted.
	// Rejected attempts are retried until the wave deadline.
...
// registerFakePeer completes the v2 transport and version/verack handshake
// with the node and then proves, with a ping/pong round trip, that the node
// processed our verack. Admission rejections surface as
...
// fakePeerConn registers one peer with the node, retrying admission
// rejections, then holds the connection open until the whole wave is ready to
// disconnect together. Only registered peers are counted toward the wave.
func fakePeerConn(nodeAddr string, handshakeSlots chan struct{},
	ready chan<- struct{}, disconnect <-chan struct{}) error {
	...
	for {
		...
		if !errors.Is(err, errRejectedBeforeVerack) ||
...
// waitForConnectionCount polls the node until its registered peer count
// matches want; registration and removal both run through the peerHandler
// goroutine, so the count is the observable barrier for NewPeer/DonePeer.
func waitForConnectionCount(client *rpcclient.Client, want int64) error {
	...
	return fmt.Errorf("timed out waiting for %d registered peers, "+
		"last count %d", want, got)
}

// runFakePeerBatch registers one full wave of peers, disconnects them
// together, and waits for every worker and for server-side peer removal.
...
// TestSyncManagerRaceCorruption stresses a single simnet node with waves of
// inbound peers that complete the handshake and disconnect together. It then
// proves the node is not stuck with a dead sync peer: a fresh node generates
// blocks and the stressed node must sync them.
func TestSyncManagerRaceCorruption(t *testing.T) {
	...
	require.GreaterOrEqualf(t, heightAfter, expected,
		"sync manager corruption after %d fake peer cycles: node stuck "+
			"with dead sync peer (height %d -> %d)",
		done, heightBefore, heightAfter)

	t.Logf("completed %d fake peer cycles; node synced (height %d -> %d)",
		done, heightBefore, heightAfter)
}

// dialPreVerackPeer exchanges version messages with the node and disconnects
// before the peer package writes verack. It returns errRejectedBeforeVerack
// when admission closed the connection before the exchange completed.
func dialPreVerackPeer(nodeAddr string) error {
	...
	p, err := peer.NewOutboundPeer(fakePeerConfig(peer.MessageListeners{
		OnVersion: func(p *peer.Peer, _ *wire.MsgVersion) *wire.MsgReject {
			// Runs on the negotiation goroutine ahead of the verack
			// write, so closing here leaves the node with a version
			// exchange and no verack.
...
// TestPreVerackDisconnect verifies that peers disconnecting after the version
// exchange but before verack (peerDone without peerAdd) leave the sync
// manager healthy.
func TestPreVerackDisconnect(t *testing.T) {
	...
	for i := 0; i < preVerackAttempts; i++ {
		...
		for {
			...
			require.Truef(t, time.Now().Before(deadline),
				"pre-verack attempt %d did not complete: %v", i+1, err)
	...
	t.Logf("node healthy after %d pre-verack disconnects (height %d -> %d)",
