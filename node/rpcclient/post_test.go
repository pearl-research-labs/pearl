...
// cancelOnReadBody is a response body that blocks reads until the request
// context is canceled, so a body read that ignores shutdown shows up as a
// symptomatic timeout instead of a hang.
...
func waitForRequestContextCancellation(ctx context.Context,
	stage string) error {
...
// sendPostShutdownScenario describes one shutdown path that must fail if the
		...
		newClient: func(cancel context.CancelCauseFunc,
			...
			return newPostModeTestClient(postRoundTripFunc(
				func(req *http.Request) (*http.Response, error) {
					...
					// Cancel through the request context so the case
					// proves propagation rather than an injected error.
					cancel(ErrClientShutdown)
...
func TestSendPostRequestWithRetrySuccess(t *testing.T) {
	...
	result, err := sendPostRequestWithRetry(
		context.Background(), newPostTestRequest(), 1, client.httpClient,
...
func TestSendPostRequestWithRetryShutdown(t *testing.T) {
	...
	for _, tt := range sendPostShutdownScenarios {
		t.Run(tt.name, func(t *testing.T) {
			...
			result, err := sendPostRequestWithRetry(
				ctx, newPostTestRequest(), tt.tries, client.httpClient,
				client.config, client.httpURL, false,
			)
...
// TestHTTPPostShutdownInterruptsPendingRequest drives a real client against a
// server that never answers, so shutdown must abort the in-flight POST.
...
// TestHTTPURLWiring guards the construction-time copy of httpURL onto the
// client; a refactor of New that drops it would leave every POST targeting "".
...
// TestSendPostRequestShutdownPrioritizesFailure repeats enough times that a
// single-select implementation choosing randomly between the ready channels
// would be caught enqueueing after shutdown.
...
func TestBatchSendErrorResolvesQueuedFutures(t *testing.T) {
	...
	// Receive is the blocking caller-facing path; the old bug never
	// resolved it, so bound the wait.
...
// TestNewBatchSerializesPostSends holds one POST open and checks that no
// second handler goroutine starts a concurrent send.
	...
	client.httpClient.Transport = postRoundTripFunc(
		func(*http.Request) (*http.Response, error) {
			...
			for {
				...
				if current <= prev ||
					atomic.CompareAndSwapInt32(&maxActive, prev, current) {
