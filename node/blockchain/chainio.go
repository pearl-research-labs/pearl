...
// DBBlockFromBytes deserializes a block from the local database. Own-DB blocks may carry leftover trailing bytes
// from older writers; rejecting them would brick startup with no recovery short of a full resync.
func DBBlockFromBytes(blockBytes []byte, hash chainhash.Hash) (*btcutil.Block, error) {
	...
	if trailing := blockReader.Len(); trailing > 0 {
		log.Debugf("Block %v has %d trailing bytes in the database; ignoring them", hash, trailing)
