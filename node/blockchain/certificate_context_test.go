...
func contextCert(t *testing.T, ancestor *wire.BlockHeader) *wire.CertificateV4 {
	t.Helper()
	delta := ancestor.IncompleteHeaderBytes()
	return &wire.CertificateV4{
		Hash:       chainhash.Hash{},
		PublicData: append(delta[:], bytes.Repeat([]byte{0xAB}, 32)...),
...
func TestIncompleteHeaderBytes(t *testing.T) {
	proposed, _ := contextHeaders(t)

	var serialized bytes.Buffer
	require.NoError(t, proposed.Serialize(&serialized))
	got := proposed.IncompleteHeaderBytes()
	require.Equal(t, serialized.Bytes()[:IncompleteBlockHeaderSize], got[:])
}
...
func TestCheckCertificateContext(t *testing.T) {
	...
	tests := []struct {
		...
		{"short public data", proposed, headers, &wire.CertificateV4{
			PublicData: make([]byte, IncompleteBlockHeaderSize-1),
		}, BFNone, true},
	}
