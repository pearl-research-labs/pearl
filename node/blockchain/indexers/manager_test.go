...
// bucketBase exists so errorBucket can embed the interface and still define
// its own Bucket method.
type bucketBase struct {
	database.Bucket
}

// errorBucket wraps a database bucket and injects errors into the operations
// dropIndex uses while cataloging and deleting index buckets.
...
// createDropIndexTestDB creates a database holding the tip entry and bucket
// dropIndex expects for idxKey.
func createDropIndexTestDB(t *testing.T, idxKey []byte) database.DB {
	t.Helper()

	db, err := database.Create(
		"ffldb", filepath.Join(t.TempDir(), "blocks_ffldb"), wire.MainNet,
	)
...
// TestDropIndexPropagatesErrors ensures failures while cataloging or deleting
// index buckets reach the caller instead of being reported as a clean drop.
