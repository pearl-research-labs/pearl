...
func TestMain(m *testing.M) {
	// logWriter forwards every subsystem log line to logRotator; admission
	// warnings emitted by these tests would otherwise hit a nil rotator.
...
// newTestServerPeer creates a serverPeer that exercises the lifecycle logic
...
func recvLifecycleEvent(t *testing.T,
	ch <-chan peerLifecycleEvent) peerLifecycleEvent {
...
// TestInboundPeerReservation pins the composition of the three budget
// functions: listener capacity derives from the peer mode while connmgr keeps
// its automatic target.
func TestInboundPeerReservation(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "connect only", maxPeers: 8, permanentPeers: 1,
			wantReserved: 1, wantInbound: 7,
		},
		{
			name: "connect only capped", maxPeers: 8, permanentPeers: 10,
			wantReserved: 8, wantInbound: 0,
		},
		{
			name: "simnet without peers", maxPeers: 8,
			wantReserved: 0, wantInbound: 8,
		},
		{
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			reserved := reservedOutboundPeers(
				tt.maxPeers, targetOutbound, tt.permanentPeers,
				tt.automatic,
			)
			assert.Equal(t, tt.wantReserved, reserved)
			assert.Equal(t, tt.wantInbound,
				maxInboundPeers(tt.maxPeers, reserved))
		})
	}
}

// TestInboundPeerAdmissionSourceLimits verifies that loopback and whitelisted
// peers keep the ordinary pending-handshake and v2 source limits; whitelisting
// only affects banning.
		...
		t.Run(tt.name, func(t *testing.T) {
			...
			for i := 0; i < 20; i++ {
				whitelisted, release, _, err :=
					s.acquireInboundPeerAdmission(tt.addr)
				if err != nil {
					break
				}
			...
			assert.Equal(t, 1, v2Rejections,
				"the v2 source rate must reject a peer")
		})
	}
}

// TestPeerLifecycleOrdering verifies that verack before disconnect yields
// peerAdd followed by peerDone, never the reverse.
...
// TestPeerLifecycleSimultaneousReady covers both channels being ready before
// the handler runs. select is nondeterministic, so peerAdd may be skipped;
// peerDone must always arrive, and after peerAdd if that was emitted.
	...
	for i := 0; i < iterations; i++ {
		...
		if first.action == peerAdd {
			...
			assert.Equal(t, peerDone, second.action,
				"iteration %d: peerAdd must be followed by peerDone", i)
			continue
		}

		assert.Equal(t, peerDone, first.action,
			"iteration %d: sole event must be peerDone", i)
	}
