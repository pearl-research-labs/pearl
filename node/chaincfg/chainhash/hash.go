...
// ErrHashStrSizeMismatch describes an error that indicates the caller specified a hash string that does not meet
// the exact length required for strict parsing.
var ErrHashStrSizeMismatch = fmt.Errorf("hash string must be exactly %d characters", MaxHashStringSize)

// Hash is used in several of the wire messages and common structures.  It
...
// NewHashFromStrStrict creates a Hash from a hash string.  The string must be the full hexadecimal string of a
// byte-reversed hash.
...
// DecodeStrict decodes the byte-reversed hexadecimal string encoding of a Hash to a destination.  The source
// string must be exactly MaxHashStringSize bytes, or ErrHashStrSizeMismatch is returned.
...
// decodeHash decodes the provided byte-reversed hexadecimal bytes into dst. Callers apply their own length
// validation first; short input is zero padded.
