...
// The handlers under test have no chain, mempool, or sync manager wired; a
// nil dereference means the trailing byte was not rejected before use.
...
func TestHandleSendRawTransactionRejectsTrailingBytes(t *testing.T) {
	...
	requireRPCErrorCode(t, err, btcjson.ErrRPCDeserialization)
	assert.Nil(t, result)
	mm.AssertNotCalled(t, "ProcessTransaction",
...
// ends, modelling zero-copy backends whose buffers die with the transaction.
...
func TestHandleGetBlockCopiesTransactionBytes(t *testing.T) {
	...
	require.NoError(t,
		chaincfg.MainNetParams.GenesisBlock.Serialize(&serializedBlock))
