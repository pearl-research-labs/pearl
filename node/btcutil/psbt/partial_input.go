...
func (pi *PInput) serialize(w io.Writer) error {
	...
	if pi.FinalScriptSig == nil && pi.FinalScriptWitness == nil {
		...
		for idx, leafScript := range pi.TaprootLeafScript {
			if leafScript == nil {
				return fmt.Errorf("nil taproot leaf script at index %d: %w", idx, ErrInvalidPsbtFormat)
			}
		}
		...
		for idx, derivation := range pi.TaprootBip32Derivation {
			if derivation == nil {
				return fmt.Errorf("nil taproot BIP32 derivation at index %d: %w", idx, ErrInvalidPsbtFormat)
			}
		}
