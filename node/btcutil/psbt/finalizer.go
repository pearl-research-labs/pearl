...
func finalizeTaprootInput(p *Packet, inIndex int) error {
	...
	for idx, scriptSpendSig := range pInput.TaprootScriptSpendSig {
		if scriptSpendSig == nil {
			return fmt.Errorf("nil taproot script spend signature at index %d: %w", idx, ErrInvalidPsbtFormat)
		}
	}
	...
	case len(pInput.TaprootScriptSpendSig) > 0:
		...
		if err != nil {
			return fmt.Errorf("control block for script spend signature not found: %w", err)
		}
		...
		witnessStack, err = taprootScriptSpendWitnessStack(leafScript.Script, pInput.TaprootScriptSpendSig)
		if err != nil {
			return err
		}
