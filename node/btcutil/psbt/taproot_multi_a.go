...
func taprootScriptSpendWitnessStack(script []byte, scriptSpendSigs []*TaprootScriptSpendSig) (wire.TxWitness, error) {
	...
	if !isMultiA {
		...
		for _, scriptSpendSig := range scriptSpendSigs {
			witnessStack = append(witnessStack, taprootScriptSpendSigBytes(scriptSpendSig))
		}
	...
	for idx, scriptSpendSig := range scriptSpendSigs {
		...
		if _, ok := keySet[key]; !ok {
			return nil, fmt.Errorf("taproot script spend signature %d does not match a multi_a key: %w",
				idx, ErrInvalidPsbtFormat)
		}
		if _, ok := sigByKey[key]; ok {
			return nil, fmt.Errorf("duplicate taproot script spend signature for multi_a key: %w",
	...
	// NUMEQUAL needs exactly threshold successful checks; extra PSBT signatures are dropped in script order so the
	// witness stays valid.
	...
	// CHECKSIG/CHECKSIGADD pop from the top of the stack, so keys are supplied in reverse script order.
...
func parseTaprootMultiA(script []byte) ([][]byte, int, bool, error) {
	...
	for tokenizer.Next() {
		token := taprootScriptToken{opcode: tokenizer.Opcode(), data: tokenizer.Data()}
	...
	if len(tokens) < 4 || len(tokens[0].data) != 32 || tokens[1].opcode != txscript.OP_CHECKSIG {
		if hasCheckSigAdd {
			return nil, 0, false, ErrUnsupportedScriptType
		}
	...
	idx := 2
	for idx+1 < len(tokens) && len(tokens[idx].data) == 32 && tokens[idx+1].opcode == txscript.OP_CHECKSIGADD {
