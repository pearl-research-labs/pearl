...
// PutPairs inserts the passed key/value pairs. Intermediate nodes from each insert are recycled so a bulk commit
// does not pin a discarded snapshot per key.
func (t *Immutable) PutPairs(kvPairs ...KVPair) *Immutable {
	...
	for _, kvPair := range kvPairs {
		...
		for _, node := range prevTreapNodes {
			...
			// Recycle only nodes unreachable from the newest snapshot; shared ancestors are still live for
			// concurrent readers.
