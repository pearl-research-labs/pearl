...
type Config struct {
	...
	// MaxInbound limits accepted inbound connections that may be active at once. A nil value is unlimited; a
	// pointer to zero disables inbound connections.
	MaxInbound *uint32

	// TargetOutbound is the number of automatic outbound network connections to maintain. Connections made
	// through Connect are additional. Defaults to 8.
...
func (cm *ConnManager) limitInbound(conn net.Conn) (net.Conn, bool) {
	...
	case cm.inboundSlots <- struct{}{}:
		return &limitedInboundConn{Conn: conn, release: func() { <-cm.inboundSlots }}, true

	default:
		rejected := atomic.AddUint64(&cm.inboundRejected, 1)
		cm.inboundLog.Do(func() {
			log.Warnf("Inbound connection limit reached: rejected=%d remote=%s", rejected, conn.RemoteAddr())
		})
