...
pearld provides a [JSON-RPC](http://json-rpc.org/wiki/specification) API.
A few key points about how RPCs are serviced:
* pearld intentionally splits the wallet and chain services into independent
  processes.  This means that if you are talking directly to pearld, only
...
* pearld is secure by default which means that the RPC connection is TLS-enabled
  by default
* pearld provides access to the API through both
  [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29) requests and
  [Websockets](http://en.wikipedia.org/wiki/WebSocket)

Websockets are the preferred transport for pearld RPC and are used by applications
such as [Oyster](https://github.com/pearl-research-labs/pearl/wallet) for inter-process
communication with pearld.  The websocket connection endpoint for pearld is
...
The pearld RPC server supports both [HTTP POST](http://en.wikipedia.org/wiki/POST_%28HTTP%29)
...
pearld is `wss://your_ip_or_domain:44107/ws`.
...
to a pearld RPC server:

* **rpcuser** is the full-access username configured for the pearld RPC server
* **rpcpass** is the full-access password configured for the pearld RPC server
* **rpclimituser** is the limited username configured for the pearld RPC server
* **rpclimitpass** is the limited password configured for the pearld RPC server
* **rpccert** is the PEM-encoded X.509 certificate (public key) that the pearld
  server is configured with.  It is automatically generated by pearld and placed
  in the pearld home directory (which is typically `%LOCALAPPDATA%\Pearld` on
  Windows and `~/.pearld` on POSIX-like OSes)

**NOTE:** As mentioned above, pearld is secure by default which means the RPC
...
The pearld RPC server uses HTTP [basic access authentication](http://en.wikipedia.org/wiki/Basic_access_authentication) with the **rpcuser**
...
pearld comes with a separate utility named `prlctl` which can be used to issue
these RPC commands via HTTP POST requests to pearld after configuring it with the
...
|25|[sendrawtransaction](#sendrawtransaction)|Y|Submits the serialized, hex-encoded transaction to the local peer and relays it to the network.<br /><font color="orange">pearld does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
|26|[setgenerate](#setgenerate) |N|Set the server to generate coins (mine) or not.<br/>NOTE: Since pearld does not have the wallet integrated to provide payment addresses, pearld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|27|[stop](#stop)|N|Shutdown pearld.|
|28|[submitblock](#submitblock)|Y|Attempts to submit a new serialized, hex-encoded block to the network.|
|29|[validateaddress](#validateaddress)|Y|Verifies the given address is valid.  NOTE: Since pearld does not have a wallet integrated, pearld will only return whether the address is valid or not.|
...
|Notes|NOTE: Since pearld does NOT contain wallet functionality, wallet-related fields are not returned.  See getinfo in Oyster for a version which includes that information.|
...
|Notes|<font color="orange">Since pearld does not perform any mining, the priority related fields `startingpriority` and `currentpriority` that are available when the `verbose` flag is set are always 0.</font>|
...
|Notes|NOTE: Since pearld does not have the wallet integrated to provide payment addresses, pearld must be configured via the `--miningaddr` option to provide which payment addresses to pay created blocks to for this RPC to function.|
|Returns|Nothing|
[Return to Overview](#MethodOverview)<br />
...
|Notes|<font color="orange">pearld does not yet implement the `allowhighfees` parameter, so it has no effect</font>|
...
|Method|stop|
|Parameters|None|
|Description|Shutdown pearld.|
|Returns|`"pearld stopping."` (string)|
[Return to Overview](#MethodOverview)<br />
...
|Description|Verifies the block chain database.<br />The actual checks performed by the `checklevel` parameter is implementation specific.  For pearld this is:<br />`checklevel=0` - Look up each block and ensure it can be loaded from the database.<br />`checklevel=1` - Perform basic context-free sanity checks on each block.|
|Notes|<font color="orange">Pearld currently only supports `checklevel` 0 and 1, but the default is still 3 for compatibility.  Per the information in the Parameters section above, higher levels are automatically clamped to the highest supported level, so this means the default is effectively 1 for pearld.</font>|
...
The following is an overview of the RPC methods which are implemented by pearld as extensions beyond the standard API. Click the method name for further details such as parameter and return information.
...
|3|[getcurrentnet](#getcurrentnet)|Y|Get Pearl network pearld is running on.|None|
...
|Parameters|None|
|Description|Get Pearl network pearld is running on.|
|Returns|numeric|
...
|Parameters|1. address (string, required) - Pearl address <br /> 2. verbose (int, optional, default=true) - specifies the transaction is returned as a JSON object instead of hex-encoded string <br />3. skip (int, optional, default=0) - the number of leading transactions to leave out of the final response <br /> 4. count (int, optional, default=100, max=10000) - the maximum number of transactions to return; values above 10000 are rejected <br /> 5. vinextra (int, optional, default=0) - Specify that extra data from previous output will be returned in vin <br /> 6. reverse (boolean, optional, default=false) - Specifies that the transactions should be returned in reverse chronological order|
...
|Method|version|
|Parameters|None|
|Description|Returns the version of the JSON-RPC API built into this release of pearld.|
...
|Description|*DEPRECATED, for similar functionality see [loadtxfilter](#loadtxfilter)*<br />Send a redeemingtx notification when a transaction spending an outpoint appears in mempool (if relayed to this pearld instance) and when such a transaction first appears in a newly-attached block.|
|Returns|Nothing|
[Return to Overview](#WSExtMethodOverview)<br />
...
pearld uses standard JSON-RPC notifications to notify clients of changes, rather than requiring clients to poll pearld for updates.  JSON-RPC notifications are a subset of requests, but do not contain an ID.  The notification type is categorized by the `method` field and additional details are sent as a JSON array in the `params` field.
...
a pearld instance via Websockets, issues [getblockcount](#getblockcount) to
...
func main() {
	...
	// generated by pearld when it starts the RPC server and doesn't already
	// have one.
...
The following is an example Go application which uses the
[rpcclient](https://github.com/pearl-research-labs/pearl/node/tree/master/rpcclient) package to connect with
a pearld instance via Websockets, issues [getblock](#getblock) to retrieve
...
The following is an example Go application which uses the
[rpcclient](https://github.com/pearl-research-labs/pearl/node/tree/master/rpcclient) package to connect with
a pearld instance via Websockets and registers for
...
(can be installed with `npm install ws`) to connect with a pearld instance,
...
// generated by pearld when it starts the RPC server and doesn't already
// have one.
var cert = fs.readFileSync('/path/to/pearld/appdata/rpc.cert');
var user = "yourusername";
var password = "yourpassword";


// Initiate the websocket connection.  The pearld generated certificate acts as
