...
// newTestSyncManager returns a sync manager over a fresh genesis-only chain.
// The params are copied so tests cannot mutate the package-level instances.
func newTestSyncManager(t *testing.T, params *chaincfg.Params) *SyncManager {
	t.Helper()

	paramsCopy := *params
	db, err := database.Create(
		"ffldb", filepath.Join(t.TempDir(), "ffldb"), paramsCopy.Net,
	)
...
// TestIsSyncCandidate pins the services a peer must advertise to be a sync
// candidate. Regtest gets no localhost exemption: an integration harness that
// wants to feed blocks has to advertise that it serves them.
