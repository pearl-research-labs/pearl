...
func TestIncompleteHeaderBytesGolden(t *testing.T) {
	header := BlockHeader{
		Version:   0x01020304,
	...
	want := [IncompleteBlockHeaderSize]byte{
		0x04, 0x03, 0x02, 0x01,
	...
	if got := header.IncompleteHeaderBytes(); got != want {
		t.Fatalf("incomplete header: got %x, want %x", got, want)
	}
	var serialized bytes.Buffer
	if err := header.Serialize(&serialized); err != nil {
		t.Fatal(err)
	}
	if !bytes.Equal(serialized.Bytes()[:IncompleteBlockHeaderSize], want[:]) {
		t.Fatal("incomplete header does not match the full header prefix")
	}
	header.ProofCommitment[0] = 0xff
	if got := header.IncompleteHeaderBytes(); got != want {
		t.Fatal("proof commitment changed the incomplete header")
	}
}
