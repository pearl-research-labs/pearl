...
// MaxMessagePayload is the maximum bytes a message can be regardless of other individual limits imposed by
// messages themselves. This is used as a serialization bound for all contexts (disk, RPC, network, etc.).
...
func ReadV2MessageN(plaintext []byte, pver uint32, enc MessageEncoding) (
	...
	if len(plaintext) > MaxProtocolMessageLength {
		str := fmt.Sprintf("message payload is too large - %d bytes, but max message payload is %d bytes",
	...
	if len(plaintext) > int(mpl) {
		str := fmt.Sprintf("payload exceeds max length - %d bytes, but max payload size for messages of "+
	...
	if buf.Len() > 0 {
		str := fmt.Sprintf("message payload has %d extra bytes after decode", buf.Len())
		return nil, nil, messageError("ReadV2MessageN", str)
	}
...
func readMessageWithEncodingNInternal(r io.Reader, pver uint32,
	...
	if pr.Len() > 0 {
		str := fmt.Sprintf("message payload has %d extra bytes after decode", pr.Len())
		return totalBytes, nil, nil, messageError("ReadMessage", str)
	}
