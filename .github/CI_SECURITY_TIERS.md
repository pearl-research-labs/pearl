...
| 2 — Maintainer-Approved | `merge_group` or `pull_request_target` + `safe-to-test` label | EC2, PAT, HF_TOKEN | GPU (EC2), `large-runner` | GPU tests, Docker builds |
...
- **`miner_ci.yml`** — Ruff checks, Pearl Gateway + Miner Base pytest
...
- **`miner_gpu_ci.yml`** — GPU tests on EC2 B200 runner (SM100-only kernels)
