...
def run_clean(tg, gp):
    # noisy_gemm's mainloop consumes ApEA/BpEB, not A/B directly. If we skip
    # noising, seed the noised operands with the clean operands so this harness
    # actually tests clean A@B instead of random ApEA@BpEB.
    tg.ApEA.copy_(tg.A)
    tg.BpEB.copy_(tg.B)
    noisy_gemm(
        A=tg.A,
...
for mc in sorted(KERNEL_CONFIGS.matmul_kernels, key=lambda c: (c.tile_size_m, c.tile_size_n, c.R)):
    ...
    try:
        run_clean(tg, gp)
        out = tg.C.cpu().to(torch.float32)
        ref = clean_ref(tg).to(torch.float32)
        d = (out - ref).abs()
        ...
        print(
            f"{tag:>14} {wgs:>4} {kwr:>9}  {'PASS' if ok else 'FAIL'}  max|d|={d.max():.2f}  rows_ok={ncorrect}/{out.shape[0]}"
        )
