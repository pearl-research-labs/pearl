...
// (128,128,128,64), kStages=3, SkipDenoising=true, SkipReduction=true.
// This is the first sm_89 .cu that gets compiled. It validates the int32
// path before noisingA/B, denoise, PoW work begins.
...
// Tile-size choice for sm_89 noiseless: bM=bN=128, bK=64, kStages=3.
//   SharedStorageNoDenoise = (smem_A + smem_B) + smem_C + scales + barriers
//                          ≈ 2 * (128*64*3 * 1B) + (128*128 * 2B) + 1 KB
//                          ≈ 49152 + 32768 + 1024 + barriers
//                          ≈ ~83 KB — fits in sm_89's 99 KB opt-in cap.
// (bK=128 would push past 132 KB and fail cudaFuncSetAttribute on sm_89.)
using NoiselessTraits128x128x64_R64 = KernelTraitsSm89<
    /*ElementIn=*/      int8_t,
    ...
    /*kStages=*/        3,
    /*EnableDebug=*/    false>;
// Explicit instantiation — produces the host launcher symbol.
template void pearl_gemm_sm89_run<NoiselessTraits128x128x64_R64>(
    typename pearl::CollectiveMainloopSm89<
        NoiselessTraits128x128x64_R64>::Arguments const& mainloop_args,
    typename pearl::CollectiveEpilogueSm89<
        NoiselessTraits128x128x64_R64>::Arguments const& epilogue_args,
...
extern "C" void pearl_gemm_sm89_noiseless_128x128x64_R64(
    ...
    int M, int N, int K,
    cudaStream_t stream) {
  using KTraits = NoiselessTraits128x128x64_R64;
...
// SkipReduction=true (no PoW accumulator), SkipDenoising=true (no denoise epilogue).
// ============================================================================

namespace pearl {
namespace sm89 {

using NoiselessTraits64x64x64_R128 = KernelTraitsSm89<
    /*ElementIn=*/      int8_t,
    /*ElementOut=*/     cutlass::bfloat16_t,
    /*ElementDenoise=*/ cutlass::half_t,
    /*ElementScale=*/   float,
...
template void pearl_gemm_sm89_run<NoiselessTraits64x64x64_R128>(
    typename pearl::CollectiveMainloopSm89<
        NoiselessTraits64x64x64_R128>::Arguments const&,
    typename pearl::CollectiveEpilogueSm89<
        NoiselessTraits64x64x64_R128>::Arguments const&,
