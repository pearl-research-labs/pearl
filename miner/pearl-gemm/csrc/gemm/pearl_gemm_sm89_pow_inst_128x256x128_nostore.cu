...
//
// sm_89 PoW instantiation at the production mining tile, MINING no-C-store
// variant (Lever A). Identical to pearl_gemm_sm89_pow_inst_128x256x128.cu
// except kMiningNoStore=true: the epilogue does NOT R2S-stage or write the
// (M,N) output C. The BLAKE3 transcript (folded over the raw int32 accumulator
// per k_block in the mainloop) is the real output, exactly as in lpminer's
// mining kernel. This lets the bench run at the TRUE mining shape
// (M=N=131072, K=4096, R=256) in 16 GB — the full C would be 32 GB — and
// removes the wasted per-attempt C bandwidth.
//
// The companion *with-store* instantiation (pearl_gemm_sm89_pow_inst_128x256x128.cu)
// is what the verify build links against so bit-exactness of the denoise + C is
// still checked.
...
using PowTraits128x256x128_R256_NoStore = KernelTraitsSm89<
    /*ElementIn=*/      int8_t,
...
template void pearl_gemm_sm89_run<PowTraits128x256x128_R256_NoStore>(
    typename pearl::CollectiveMainloopSm89<
        PowTraits128x256x128_R256_NoStore>::Arguments const&,
...
extern "C" void pearl_gemm_sm89_pow_128x256x128_R256_nostore(
    ...
    int M, int N, int K,
    cudaStream_t stream) {
  using KTraits = PowTraits128x256x128_R256_NoStore;
