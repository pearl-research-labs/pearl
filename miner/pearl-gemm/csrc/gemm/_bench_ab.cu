// A/B benchmark: same bench harness, runs both:
//   pearl_gemm_sm89_noiseless_128x128x64_R64           (NEW: PersistentSwizzled)
//   pearl_gemm_sm89_noiseless_128x128x64_R64_baseline  (BASELINE: SimpleTileScheduler)
//
// And the denoise pair. Side-by-side output for direct comparison.
...
extern "C" void pearl_gemm_sm89_denoise_128x128x64_R64(
    ...
    cutlass::half_t const*, cutlass::half_t const*,
    int, int, int, cudaStream_t);
extern "C" void pearl_gemm_sm89_denoise_128x128x64_R64_baseline(
    int8_t const*, int64_t, int8_t const*, int64_t,
    cutlass::bfloat16_t*, int64_t, float const*, float const*,
    cutlass::half_t const*, cutlass::half_t const*,
    cutlass::half_t const*, cutlass::half_t const*,
...
int main(int argc, char** argv) {
    ...
    int iters[]   = { 50, 30, 10, 10, 5, 5, 5 };

    printf("\n%-32s  | %-10s | %-10s | %-7s\n", "noiseless GEMM TOPS", "BASELINE", "NEW", "ratio");
    printf("%-32s  +-%-10s-+-%-10s-+-%-7s\n", "--------------------------------", "----------", "----------", "-------");
    for (size_t i = 0; i < sizeof(iters)/sizeof(iters[0]); ++i) {
        int M = sizes[i][0], N = sizes[i][1], K = sizes[i][2];
        double bl = bench_noiseless(pearl::sm89::pearl_gemm_sm89_noiseless_128x128x64_R64_baseline, M, N, K, iters[i]);
        double nw = bench_noiseless(pearl::sm89::pearl_gemm_sm89_noiseless_128x128x64_R64,          M, N, K, iters[i]);
        printf("M=%5d N=%5d K=%5d         | %10.2f | %10.2f | %.2fx\n", M, N, K, bl, nw, nw / bl);
    }

    int eiters[]  = { 20, 10, 5, 5, 3, 3, 3 };
    printf("\n%-32s  | %-10s | %-10s | %-7s\n", "noisy_gemm e2e (main TOPS)", "BASELINE", "NEW", "ratio");
    printf("%-32s  +-%-10s-+-%-10s-+-%-7s\n", "--------------------------------", "----------", "----------", "-------");
    for (size_t i = 0; i < sizeof(eiters)/sizeof(eiters[0]); ++i) {
        int M = sizes[i][0], N = sizes[i][1], K = sizes[i][2];
        double bl = bench_e2e_denoise(pearl::sm89::pearl_gemm_sm89_denoise_128x128x64_R64_baseline, M, N, K, eiters[i]);
        double nw = bench_e2e_denoise(pearl::sm89::pearl_gemm_sm89_denoise_128x128x64_R64,          M, N, K, eiters[i]);
        printf("M=%5d N=%5d K=%5d         | %10.2f | %10.2f | %.2fx\n", M, N, K, bl, nw, nw / bl);
    }
    return 0;
}
