...
print(f"Config: {mc.tile_size_m}x{mc.tile_size_n}x{mc.tile_size_k} stage={mc.pipeline_stages} R={mc.R}\n")
m, n, k = 128, 128, 256
...
def noised_ref(tg):
    ...
    A = tg.A.clone().to(torch.int32); B = tg.B.clone().to(torch.int32)
...
def run(tg, gp, skip_denoising):
    noisy_gemm(
        ...
        skip_reduction=gp.skip_reduction, skip_denoising=skip_denoising,
        pow_target=tg.pow_target, pow_key=tg.pow_key,
    )
    torch.cuda.synchronize()

def report(tag, out, ref):
    out = out.to(torch.float32); ref = ref.to(torch.float32)
    d = (out - ref).abs()
    ok = torch.allclose(out, ref, atol=1e-1, rtol=1e-2)
    print(f"[{tag}] {'PASS' if ok else 'FAIL'}  max|d|={d.max():.3f} mean|d|={d.mean():.4f}  "
          f"ref|.|mean={ref.abs().mean():.3f} out|.|mean={out.abs().mean():.3f}")
    return ok

def make(skip_a, skip_b):
    gp = GEMMParam(m, n, k, skip_noising_a=skip_a, skip_noising_b=skip_b,
...
gp, tg = make(True, True); run(tg, gp, skip_denoising=False); report("T1 clean", tg.C.cpu(), clean_ref(tg))

print("\n=== T5: full path (baseline repro) vs clean ref ===")
gp, tg = make(False, False); run(tg, gp, skip_denoising=False); report("T5 full", tg.C.cpu(), clean_ref(tg))

print("\n=== T2: full noise but skip_denoising=True vs NOISED ref ===")
gp, tg = make(False, False); run(tg, gp, skip_denoising=True); report("T2 nodenoise", tg.C.cpu(), noised_ref(tg))

print("\n=== T3: noise A only (skip B), denoise on, vs clean ref ===")
gp, tg = make(False, True); run(tg, gp, skip_denoising=False); report("T3 Aonly", tg.C.cpu(), clean_ref(tg))

print("\n=== T4: noise B only (skip A), denoise on, vs clean ref ===")
gp, tg = make(True, False); run(tg, gp, skip_denoising=False); report("T4 Bonly", tg.C.cpu(), clean_ref(tg))
