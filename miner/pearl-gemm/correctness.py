...
def compute_ref_tensor(tg):
    A = tg.A.clone(); B = tg.B.clone()
    As = tg.A_scales.clone(); Bs = tg.B_scales.clone()
...
for mc in KERNEL_CONFIGS.matmul_kernels:
    for (m, n, k) in [(128, 128, 256), (1024, 1024, 512), (1025, 1032, 512), (8192, 6144, 4096)]:
        for dt in ["fp16", "int32"]:
            try:
                gp = GEMMParam(m, n, k, skip_noising_a=False, skip_noising_b=False,
                               EARxBpEB_type_noising=dt, AxEBL_type_noising=dt,
